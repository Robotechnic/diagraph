...
size_t LabelInfo_size(const void *s){
	return TYPST_INT_SIZE + TYPST_INT_SIZE + TYPST_INT_SIZE + strlen(((LabelInfo*)s)->fontName) + 1 + TYPST_INT_SIZE;
}
...
void free_graphInfo(graphInfo *s) {
    for (size_t i = 0; i < s->manualLabels_len; i++) {
    free_LabelInfo(&s->manualLabels[i]);
    }
    free(s->manualLabels);
    for (size_t i = 0; i < s->nativeLabels_len; i++) {
    free_LabelInfo(&s->nativeLabels[i]);
    }
    free(s->nativeLabels);
    if (s->svg) {
        free(s->svg);
...
size_t graphInfo_size(const void *s){
	return 1 + TYPST_INT_SIZE + list_size(((graphInfo*)s)->manualLabels, ((graphInfo*)s)->manualLabels_len, LabelInfo_size, sizeof(*((graphInfo*)s)->manualLabels)) + TYPST_INT_SIZE + list_size(((graphInfo*)s)->nativeLabels, ((graphInfo*)s)->nativeLabels_len, LabelInfo_size, sizeof(*((graphInfo*)s)->nativeLabels)) + strlen(((graphInfo*)s)->svg) + 1;
}
int encode_graphInfo(const graphInfo *s) {
    ...
    for (size_t i = 0; i < s->manualLabels_len; i++) {
        if ((err = encode_LabelInfo(&s->manualLabels[i], __input_buffer + __buffer_offset, &buffer_len, &__buffer_offset))) {
            return err;
        }
    }
    INT_PACK(s->nativeLabels_len)
    for (size_t i = 0; i < s->nativeLabels_len; i++) {
        if ((err = encode_LabelInfo(&s->nativeLabels[i], __input_buffer + __buffer_offset, &buffer_len, &__buffer_offset))) {
            return err;
        }
    }
    STR_PACK(s->svg)
