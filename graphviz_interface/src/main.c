 ...
 */
void get_label_positions(graph_t *g, double pad, bool *consider, uint8_t *buffer) {
    int label_count = 0;
 ...
 */
EMSCRIPTEN_KEEPALIVE
int render(size_t font_size_len, size_t dot_len, size_t label_dimensions_len, size_t engine_len) {
    // Set error report to custom function. Lets us get error messages from Graphviz.
    agseterr(AGERR);
    agseterrf(vizErrorf);

    INIT_BUFFER_UNPACK(font_size_len + dot_len + label_dimensions_len + engine_len);
    double font_size;
    ...
    // Passing `NULL` sets a default value. Passing a graph sets the value for the graph.
    ...
    {
        double pad_inches;
        sscanf(agget(g, "pad"), "%lf", &pad_inches);
    ...
    get_label_positions(g, pad, is_label_overridden, label_positions);
